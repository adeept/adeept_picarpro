...
async def recv_msg(websocket):
    global speed_set, modeSelect, data
    move.setup()
    direction_command = 'no'
    turn_command = 'no'
...
def motor_stop():
    ...
    while True:
        ...
        # print("data:",data)
        while ("forward" in data or "backward" in data) or data == "get_info" :
            # print("start")
            time_end = time.time()
            if time_end-time_start >= 1.5:  # 1.5s后电机停止,可以改值.
                # print("stop")
                move.move(speed_set, 'no', 'no', rad)
                break
            elif data == "DS":
...
if __name__ == '__main__':
    ...
    testNC_threading=threading.Thread(target=motor_stop)
    testNC_threading.setDaemon(False)
    testNC_threading.start()    
