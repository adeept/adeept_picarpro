...
def findLineCtrl(posInput, setCenter):#2
	global findLineMove,tracking_servo_status,FLCV_Status,tracking_servo_left,tracking_servo_left_mark,tracking_servo_right_mark,servo_left_stop,servo_right_stop
	if FLCV_Status == 0:    
	...
	if posInput != None and findLineMove == 1:
		...
		if tracking_servo_status == -1 :
			angle_Limit = scGear.returnServoAngle(0) 
			print(angle_Limit)
			if angle_Limit > 20: 
				scGear.moveAngle(0, -30 * Dv) 
				move.video_Tracking_Move(turn_speed, 1) 
